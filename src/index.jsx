...
export class FormGenerator extends Component {
	...
	constructor(props) {
		...
		this.isVisible = this.constructor.isVisible.bind(this.constructor);
		this.getAllVisible = this.constructor.getAllVisible.bind(this.constructor);
		this.getVisibleFields = this.constructor.getVisibleFields.bind(this.constructor);
		this.getFieldComplete = this.constructor.getFieldComplete.bind(this.constructor);
		this.getFieldsComplete = this.constructor.getFieldsComplete.bind(this.constructor);
		this.getVisibleOptions = this.constructor.getVisibleOptions.bind(this.constructor);
		this.getRequiredFields = this.constructor.getRequiredFields.bind(this.constructor);
		this.getInvalidateFields = this.constructor.getInvalidateFields.bind(this.constructor);
		this.getFieldValidatorsBinders = this.constructor.getFieldValidatorsBinders.bind(this.constructor);
	...
	static isVisible(checker, data) {
		...
		return new Function('data', `return ${ checker };`).call(this, data);
	}

	static getFieldsComplete(schema, data) {

		const invalidateFields = this.getInvalidateFields(schema, data);

		if (invalidateFields.length) log('FormGenerator -> getFieldsComplete', { invalidateFields });

		else return true;
	}

	static getInvalidateFields(schema, data) {
		return this.getRequiredFields(schema, data).filter(field => !data[field.name] || !this.getFieldComplete(data, field));
	}

	static getFieldComplete(data, field) {

		return data[field.name] && this.getFieldValidatorsBinders(field.validations).reduce((memo, binder) => {
			return memo && !binder({ props: { data } })(data[field.name]);
		}, true);
	}
	...
	static getRequiredFields(schema, data) {

		return this.getVisibleFields(schema, data).filter(field => {
			return field.validations && field.validations.indexOf('required') !== -1;
		});
	}

	static getFieldValidatorsBinders(fieldValidations) {

		if (fieldValidations) return fieldValidations.map(validation => {
	...
	getBindedFieldValidators(fieldValidations) {
		if (fieldValidations) return this.getFieldValidatorsBinders(fieldValidations).map(binder => binder(this));
	}
